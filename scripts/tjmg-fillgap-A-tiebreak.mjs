/**
 * tjmg-fillgap-A-tiebreak.mjs
 *
 * Replicação do método STJ fillgap (20/abr) para o TJMG.
 * Cobre eventual gap rerrodando o primary com sort COMPOSTO
 * [@timestamp asc, id.keyword asc] — paginação determinística.
 *
 * Output: G:/datajud_raw/nivel_2_regionais/estadual/TJMG_repass_A/
 *
 * Princípio: nada descartado. Soma ao raw existente; dedup por _id em carga DuckDB.
 */
...
const SIGLA = 'TJMG';
const OUT = 'G:/datajud_raw/nivel_2_regionais/estadual/TJMG_repass_A';
...
let pages = 0;
console.log(`[${SIGLA} A-tiebreak] start (file_index=${chk.file_index}, fetched=${chk.total_fetched})`);
...
while (true) {
  ...
  try {
    ...
    console.error(`[${SIGLA} A-tiebreak] ERRO: ${e.message}`);
    process.exit(1);
  }
...
writeFileSync(MAN, JSON.stringify({
  alias: ALIAS, sigla: SIGLA, mode: 'fillgap_A_tiebreak',
...
console.log(`[${SIGLA} A-tiebreak] FIM: ${chk.total_fetched.toLocaleString('pt-BR')} em ${Math.round((Date.now()-t0)/1000)}s`);
