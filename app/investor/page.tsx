...
function InvestorContent() {
  ...
  const [status, setStatus] = useState<'loading' | 'granted' | 'denied'>(token ? 'loading' : 'denied')
  ...
  return (
    <Suspense fallback={
      <div style={{ background: '#0d1f35', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 'clamp(4rem,15vw,6rem)', fontWeight: 700, color: '#ffffff', letterSpacing: '.02em' }}>Jud<span style={{ color: '#c8922a' }}>X</span></div>
      </div>
    }>
      <InvestorContent />
    </Suspense>
